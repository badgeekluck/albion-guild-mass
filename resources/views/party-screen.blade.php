...
<head>
    ...
    <style>
        ...
        .main-container { display: flex; gap: 20px; width: 98%; margin: 0 auto; align-items: flex-start; overflow: hidden; }
        @media (max-width: 1400px) { .slot-note-fixed { font-size: 10px; max-width: 80px; } }
        .slot-note-fixed { font-size: 11px; color: #fbbf24; font-weight: 600; font-style: italic; margin-left: auto; padding-left: 10px; white-space: nowrap; max-width: 120px; overflow: hidden; text-overflow: ellipsis; opacity: 0.9; }
        .role-input { width: 100%; padding: 10px; margin: 5px 0 15px 0; background: #1e1e24; border: 1px solid #444; color: white; border-radius: 4px; }
        .role-input:focus { border-color: #6366f1; outline: none; }
        .role-input:disabled { background: #111; color: #444; border-color: #222; cursor: not-allowed; }
        .role-input.exclusive-active { border: 2px solid #ef4444 !important; color: #ef4444 !important; font-weight: bold; box-shadow: 0 0 10px rgba(239, 68, 68, 0.2); }
        .my-own-slot { border: 3px solid #fbbf24 !important; box-shadow: 0 0 15px rgba(251, 191, 36, 0.6); transform: scale(1.02); z-index: 10; position: relative; }
        .you-badge { background: #fbbf24; color: #000; font-size: 10px; font-weight: bold; padding: 2px 6px; border-radius: 4px; margin-right: 8px; text-transform: uppercase; }
        .role-warning { color: #ef4444 !important; font-weight: bold; display: flex; align-items: center; gap: 4px; }
        .warning-icon { font-size: 12px; animation: pulse 2s infinite; }
        @keyframes pulse { 0% { opacity: 1; } 50% { opacity: 0.5; } 100% { opacity: 1; } }
        @keyframes redPulse { 0% { box-shadow: 0 0 5px rgba(239, 68, 68, 0.4); border-color: #ef4444; } 50% { box-shadow: 0 0 15px rgba(239, 68, 68, 0.8); border-color: #b91c1c; } 100% { box-shadow: 0 0 5px rgba(239, 68, 68, 0.4); border-color: #ef4444; } }
        .roster-area { flex: 1; display: flex; flex-direction: column; height: 100%; }
        .parties-grid { display: flex; flex-wrap: nowrap; gap: 15px; align-items: flex-start; padding-bottom: 10px; overflow-x: auto; height: 100%; }
        .party-column { flex: 0 0 340px; background: #25252e; border: 1px solid #333; border-radius: 8px; padding: 8px; display: flex; flex-direction: column; max-height: 100%; }
        .slots-container { overflow-y: auto; flex-grow: 1; padding-right: 4px; }
        .slots-container::-webkit-scrollbar { width: 4px; }
        .slots-container::-webkit-scrollbar-track { background: #1e1e24; }
        .slots-container::-webkit-scrollbar-thumb { background: #4f46e5; border-radius: 10px; }
        .party-header { text-align: center; font-weight: bold; color: #6366f1; border-bottom: 2px solid #333; padding-bottom: 8px; margin-bottom: 10px; text-transform: uppercase; letter-spacing: 1px; }
        .party-slot { display: flex; align-items: center; margin-bottom: 6px; padding: 0 10px; height: 44px; border-radius: 4px; transition: all 0.2s; position: relative; font-size: 14px; cursor: pointer; }
        ...
        .role-bombsquad { background-color: rgba(220, 38, 38, 0.15) !important; border: 2px solid #ef4444 !important; animation: redPulse 2s infinite; }
        .role-bombsquad .slot-role { background: #ef4444; color: white; font-weight: bold; }
        .sidebar { width: 300px; background: #2b2b36; padding: 20px; border-radius: 8px; max-height: 100%; display: flex; flex-direction: column; overflow: hidden; }
        .waitlist-item { background: #383844; padding: 8px; margin-bottom: 6px; border-radius: 4px; display: flex; flex-direction: column; gap: 4px; border: 1px solid transparent; }
        .waitlist-item.bombsquad-wait { border: 1px solid #ef4444; background: rgba(239, 68, 68, 0.1); }
        .waitlist-item.bombsquad-wait .tag-main { background-color: #ef4444 !important; animation: pulse 1.5s infinite; }
        .waitlist-area { flex-grow: 1; overflow-y: auto; max-height: 600px; min-height: 150px; border: 2px dashed #444; border-radius: 6px; padding: 10px; }
        ...
        @media (max-width:1600px) { .roster-area{overflow-y: auto; max-width: 1115px;} }
        @media (max-width: 767px) { .sidebar{width:100%; order:1;} .roster-area{order:2;} .main-container {flex-wrap:wrap;} }
        ...
        @media (max-width: 767px) { .header-wrapper{gap:10px; flex-wrap:wrap;} .header-wrapper .header-login, .header-wrapper .header-auth{margin-top:20px; padding-top: 20px; border-top: 1px solid #444; margin-left:0; padding-left:0; border-left:none; width:100%;} .header-wrapper .header-icon{padding:4px; font-size:12px;} }
        .albion-equipment-grid { display: grid; grid-template-columns: 80px 80px 80px; grid-template-rows: 80px 80px 80px; gap: 12px; justify-content: center; background: #2b2b36; padding: 25px; border-radius: 8px; border: 2px solid #444; margin-top: 15px; grid-template-areas: ". head cape" "wep arm off" "pot shoe food"; }
        .area-head { grid-area: head; } .area-cape { grid-area: cape; } .area-wep  { grid-area: wep; } .area-arm  { grid-area: arm; } .area-off  { grid-area: off; } .area-pot  { grid-area: pot; } .area-shoe { grid-area: shoe; } .area-food { grid-area: food; }
        .eq-slot { background: #1e1e24; border: 1px solid #555; border-radius: 6px; position: relative; display: flex; align-items: center; justify-content: center; box-shadow: inset 0 0 10px rgba(0,0,0,0.5); }
        ...
        #eq-head { grid-column: 2; grid-row: 1; } #eq-cape { grid-column: 3; grid-row: 1; } #eq-armor { grid-column: 2; grid-row: 2; } #eq-weapon { grid-column: 1; grid-row: 2; } #eq-offhand { grid-column: 3; grid-row: 2; } #eq-shoe { grid-column: 2; grid-row: 3; }
        #viewer-list-tooltip { display: none; position: absolute; top: 110%; right: 0; background: #25252e; border: 1px solid #444; border-radius: 8px; padding: 15px; max-width: 500px; z-index: 1000; box-shadow: 0 10px 25px rgba(0,0,0,0.5); }
        #viewer-names { list-style: none; padding: 0; margin: 0; padding-bottom: 8px; display: flex; flex-wrap: nowrap; overflow-x: auto; overflow-y: hidden; gap: 8px; }
        #viewer-names::-webkit-scrollbar { height: 6px; } #viewer-names::-webkit-scrollbar-track { background: #1e1e24; border-radius: 4px; } #viewer-names::-webkit-scrollbar-thumb { background: #4f46e5; border-radius: 4px; }
        .viewer-tag { background: rgba(255, 255, 255, 0.05); border: 1px solid #444; border-radius: 4px; padding: 4px 8px; font-size: 12px; color: #ccc; display: flex; align-items: center; gap: 6px; transition: all 0.2s; cursor: default; white-space: nowrap; flex-shrink: 0; }
        .viewer-tag:hover { background: rgba(99, 102, 241, 0.2); border-color: #6366f1; color: white; }
        .party-count-badge { font-size: 11px; background: #1e1e24; padding: 2px 8px; border-radius: 6px; border: 1px solid #444; margin-left: 8px; color: #fbbf24; font-weight: bold; letter-spacing: 0.5px; }
        .is-empty-slot { background-color: rgba(25, 25, 30, 0.4) !important; border: 1px dashed #555 !important; opacity: 0.7; }
        .is-empty-slot:hover { opacity: 1; border-color: #6366f1 !important; background-color: rgba(255, 255, 255, 0.05) !important; }
        .is-empty-slot.role-tank { border-left: 3px solid #2563eb !important; } .is-empty-slot.role-heal { border-left: 3px solid #16a34a !important; } .is-empty-slot.role-dps  { border-left: 3px solid #dc2626 !important; } .is-empty-slot.role-supp { border-left: 3px solid #d97706 !important; }
    </style>
</head>
<body>
...
<div class="header-wrapper">
    ...
    <div class="header-item" style="position: relative; cursor: pointer;" id="viewer-container">
        ...
        <div id="viewer-list-tooltip" style="display: none; position: absolute; top: 100%; right: 0; background: #25252e; border: 1px solid #444; border-radius: 6px; padding: 10px; min-width: 150px; z-index: 100; box-shadow: 0 4px 15px rgba(0,0,0,0.5); margin-top: 10px;">
            <div style="font-size: 10px; color: #888; margin-bottom: 5px; border-bottom: 1px solid #444; padding-bottom: 3px;">ONLINE USERS</div>
            <ul id="viewer-names" style="list-style: none; padding: 0; margin: 0; font-size: 12px; color: #ccc; max-height: 200px; overflow-y: auto;"></ul>
        </div>
    </div>
    @auth
        <div class="header-auth">
            <div style="font-size: 12px; color: #ccc; margin-bottom: 6px;">{{ auth()->user()->name }}</div>
            <div style="display: flex; gap: 8px;">
                @if(in_array(auth()->user()->role, ['admin', 'content-creator']))
                    <a href="{{ route('dashboard') }}" style="color: #6366f1; background: rgba(99, 102, 241, 0.1); font-size: 11px; text-decoration: none; font-weight: bold; border: 1px solid #6366f1; padding: 3px 10px; border-radius: 4px; transition: all 0.2s;">⬅ DASHBOARD</a>
                @endif
                <a href="{{ route('logout') }}" style="color: #ef4444; background: rgba(239, 68, 68, 0.1); font-size: 11px; text-decoration: none; font-weight: bold; border: 1px solid #ef4444; padding: 3px 10px; border-radius: 4px;">LOGOUT</a>
            </div>
        </div>
    @else
        <div class="header-login">
            <a href="{{ route('login') }}" style="background: #5865F2; color: white; text-decoration: none; padding: 8px 16px; border-radius: 4px; font-size: 12px; font-weight: bold;">Login with Discord</a>
        </div>
    @endauth
</div>
...
<div class="main-container">
    ...
    @if(!request('ajax'))
</div> <div id="joinModal" class="modal" onclick="if(event.target==this)this.style.display='none'">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function checkRules(e) {
            ...
            if (isCta) {
                ...
                if (isValid) {
                    submitBtn.disabled = false; submitBtn.innerText = "Sign Up"; submitBtn.style.opacity = '1';
                } else {
                    submitBtn.disabled = true; submitBtn.style.opacity = '0.5';
                    if (hasDuplicates) { submitBtn.innerText = "⚠️ Duplicate Roles Selected"; }
                    else if (selectedCount === 1 && !hasFill) { submitBtn.innerText = "Select 1 More Role"; }
        ...
        if(container && tooltip) {
            ...
            function hideTooltip() { hideTimeout = setTimeout(() => { tooltip.style.display = 'none'; }, 300); }
            container.addEventListener('mouseenter', showTooltip); container.addEventListener('mouseleave', hideTooltip);
            tooltip.addEventListener('mouseenter', showTooltip); tooltip.addEventListener('mouseleave', hideTooltip);
        }
